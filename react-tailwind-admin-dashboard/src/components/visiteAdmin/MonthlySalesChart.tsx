...
import {fetchMonthlyTours} from "../../services/TourServices.js";
...
export default function MonthlySalesChart() {
  ...
  useEffect(() => {
    const fetchMonthlyToursfct = async () => {
      try {
        const response = await fetchMonthlyTours();
        if (!response) {
          throw new Error(`HTTP error! status: ${response}`);
        }
        const data: MonthlyTourData = response;
        setChartData(data);
        setLoading(false);
      } catch (e: any) {
        setError(e.message);
